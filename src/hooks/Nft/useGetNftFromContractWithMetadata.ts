...
import type { IGetNftFromContractWithMetadata } from "../../types";
import { CovalentClient } from "@covalenthq/client-sdk";
import { CovalentAPIContext } from "../../providers";
import { revalidateTime } from "../../helpers";

export const useGetNftFromContractWithMetadata = ({
    chainName,
    contractAddress,
    options,
    revalidate = true,
}: IGetNftFromContractWithMetadata) => {
    ...
    const refetch = useCallback(
        async (isRevalidate = false) => {
            ...
            await apiService.NftService.getTokenIdsForContractWithMetadataByPage(
                chainName,
                contractAddress,
                options
            ).then((data) => {
...
export default useGetNftFromContractWithMetadata;
