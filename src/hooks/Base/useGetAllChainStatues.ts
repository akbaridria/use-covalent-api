...
import type { AllChainsStatusResponse } from "@covalenthq/client-sdk";
import { CovalentClient } from "@covalenthq/client-sdk";
import { CovalentAPIContext } from "../../providers";
import { revalidateTime } from "../../helpers";

export const useGetAllChainStatuses = ({
    revalidate = true,
}: {
    revalidate: boolean;
}) => {
    const [data, setData] = useState<AllChainsStatusResponse>();
    ...
    const refetch = useCallback(async (isRevalidate = false) => {
        ...
        await apiService.BaseService.getAllChainStatus().then((data) => {
...
export default useGetAllChainStatuses;
